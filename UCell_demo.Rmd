...
colnames(time_table) <- c("size","AUCell","Frank")

memory_table <- matrix(NA,nrow = length(testSamp),ncol = 3)
colnames(memory_table) <- c("size","AUCell","Frank")
...
for (i in seq_along(testSamp)[]){
  ...
  Rprof(tf <- "rprof.log", memory.profiling=TRUE)
  t <- system.time({
    obj_out2 <- AddModuleScore_UCell(obj, features = features, maxRank = 1000)
  })
  Rprof(NULL)
...
#time_table.df2 <- time_table %>% as.data.frame() %>% mutate(diff=AUCell-Frank)
