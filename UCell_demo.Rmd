...
head(pbmc.Tcell.UcellPrecomp@misc$UCell$RNA$cells_rankings[rownames(pbmc.Tcell.UcellPrecomp) == "CD8A",1:10])
hist(as.numeric(pbmc.Tcell.UcellPrecomp@misc$UCell$RNA$cells_rankings[rownames(pbmc.Tcell.UcellPrecomp) == "CD8A",]))
```
...
```{r}
testSamp <- c(1e3,2e3,3e3,4e3,5e3)
#testSamp <- c(1e3,5e3,1e4)
...
for (i in seq_along(testSamp)[]){
  ...
  Rprof(tf <- "rprof.log", memory.profiling=TRUE)
  t <- system.time({
    obj_out2 <- AddModuleScore_UCell(obj_UCell, features = features, maxRank = 1000)
  })
  Rprof(NULL)
