...
Small benchmark - using gc()
```{r eval=T}
#testSamp <- c(100,1000,2000,3000,4000)
testSamp <- c(100,1000,2000,3000,4000,5000,9000)
chunk.size <- 1000
...
for (i in seq_along(testSamp)[]){
  ...
  gc1 <- gc(reset = TRUE)
  t <- system.time({
    obj_out2 <- AddModuleScore_UCell(obj[,1:i.samp], features = features, chunk.size = chunk.size)
  })
  gc2 <- gc()
  memPeak <- sum(gc2[,7]) - sum(gc1[,7])
...
pbmc.azimuth.full[["RNA"]] <- CreateAssayObject(counts = pbmc.azimuth.full@assays$SCT@counts)

DefaultAssay(pbmc.azimuth.full) <- "RNA"
pbmc.azimuth.full <- DietSeurat(pbmc.azimuth.full, counts=F, data=T, assays = "RNA", graphs=F, dimreducs = F)

```

AUCell breaks with 150,000 cells - limit to 100,000
```{r eval=T}
library(tidyr)
#testSamp <- c(100,1000,2000,3000)
testSamp <- c(100, 1000, 2000, 5000, 1e4, 2e4, 1e5)
chunk.size <- 1000

features <- HCA.markers.Hs.Tcell
obj <- pbmc.azimuth.full
...
ggplot(time_table.df, aes(x=size, y = time, name)) +  geom_point(aes(color = name))
ggsave("plots/benchmark_time_direct.large.png", height=4, width=5)
memory_table.df <- memory_table %>% as.data.frame() %>% pivot_longer(-size,values_to="memory") 

ggplot(memory_table.df, aes(x=size, y = memory, name)) +  geom_point(aes(color = name)) + ylim(c(0,NA))
ggsave("plots/benchmark_mem_direct.large.png", height=4, width=5)
